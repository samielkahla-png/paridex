...
function buildCombos(selections, risk, maxCombos) {
  const settings = {
    safe: { sizes: [2], maxOdd: 4.5, minScore: 58 },
    mod: { sizes: [2, 3], maxOdd: 8.5, minScore: 48 },
    bold: { sizes: [3, 4], maxOdd: 18, minScore: 38 },
  }[risk] || { sizes: [2, 3], maxOdd: 8.5, minScore: 48 };

  const pool = selections
    .filter((s) => s.score >= settings.minScore)
  ...
  for (const size of settings.sizes) {
    for (const legs of combinations(pool, size)) {
      ...
      if (odd > settings.maxOdd) continue;

      const avgScore = legs.reduce((sum, l) => sum + l.score, 0) / legs.length;
      const impliedProbability = legs.reduce((acc, l) => acc * l.impliedProbability, 1);
      const dataQuality = legs.every((l) => l.dataQuality === 'full') ? 'full' : 'partial';

      combos.push({
        id: key,
        title: size === 2 ? 'Combiné maîtrisé' : size === 3 ? 'Combiné équilibré' : 'Combiné audacieux',
        legs,
  ...
  return combos
    .sort((a, b) => (b.confidence * 0.7 + Math.log(b.odd) * 12) - (a.confidence * 0.7 + Math.log(a.odd) * 12))
    .slice(0, maxCombos);
}
